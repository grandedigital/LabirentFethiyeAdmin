...
const getInitialValues = (selectedItem, selectedLanguage, newData) => {
    const data = selectedItem?.categoryDetails?.find((itm) => itm.languageCode === selectedLanguage)
    if (data) {
...
export default function FormCategoryUpdate({ closeModal, setIsAdded, selectedItem }) {
    const user = useUser()
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                const id = selectedItem?.categoryDetails?.find((itm) => itm.languageCode === selectedLanguage)?.id
                ...
                if (id) {
                    fd.append('Id', id)
                    await UpdateCategoryDetail(fd).then((res) => {
                    ...
                    fd.append('CategoryId', selectedItem?.id)
                    fd.append('LanguageCode', selectedLanguage)
                    await CreateCategoryDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Kategori detayları eklendi.',
                                variant: 'alert',
                                ...
                                <Tabs value={value} onChange={handleChange} variant="scrollable" scrollButtons="auto" aria-label="account profile tab">
                                    {
                                        user?.config?.companyLanguages.map((itm, i) => {
                                            return (
                                                <Tab key={i} label={itm} style={{ color: selectedItem?.categoryDetails?.find((item) => item.languageCode === itm) ? '#107d4f' : '#d35a00' }} component={Typography} icon={<LanguageCircle />} iconPosition="start" />
                                            )
                                        })
                                    }
                                </Tabs>
