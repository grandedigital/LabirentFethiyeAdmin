...
const getInitialValues = (selectedItem, selectedLanguage, newData) => {
    const data = selectedItem?.villaDetails?.find((itm) => itm.languageCode === selectedLanguage)
    if (data) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                const id = selectedItem?.villaDetails?.find((itm) => itm.languageCode === selectedLanguage)?.id
                ...
                if (id) {
                    ...
                    await VillaUpdateDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Villa düzenlendi.',
                                variant: 'alert',
                    ...
                    fd.append('VillaId', selectedItem?.id)
                    fd.append('LanguageCode', selectedLanguage)
                    await VillaCreateDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Villa detayları eklendi.',
                                variant: 'alert',
                                ...
                                <Tabs value={value} onChange={handleChange} variant="scrollable" scrollButtons="auto" aria-label="account profile tab">
                                    {
                                        user?.config?.companyLanguages.map((itm, i) => {
                                            return (
                                                <Tab key={i} label={itm} style={{ color: selectedItem?.villaDetails?.find((item) => item.languageCode === itm) ? '#107d4f' : '#d35a00' }} component={Typography} icon={<LanguageCircle />} iconPosition="start" />
                                            )
                                        })
                                    }
                                </Tabs>
