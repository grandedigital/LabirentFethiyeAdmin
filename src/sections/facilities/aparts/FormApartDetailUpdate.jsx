...
const getInitialValues = (selectedItem, selectedLanguage, newData) => {
    const data = selectedItem?.hotelDetails?.find((itm) => itm.languageCode === selectedLanguage)
    if (data) {
...
export default function FormApartDetailUpdate({ closeModal, setIsAdded, selectedItem }) {
    const user = useUser()
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                const id = selectedItem?.hotelDetails?.find((itm) => itm.languageCode === selectedLanguage)?.id
                ...
                if (id) {
                    ...
                    await UpdateApartDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Apart düzenlendi.',
                                variant: 'alert',
                    ...
                    fd.append('HotelId', selectedItem?.id)
                    fd.append('LanguageCode', selectedLanguage)
                    await CreateApartDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Apart detayları eklendi.',
                                variant: 'alert',
                                ...
                                <Tabs value={value} onChange={handleChange} variant="scrollable" scrollButtons="auto" aria-label="account profile tab">
                                    {
                                        user?.config?.companyLanguages.map((itm, i) => {
                                            return (
                                                <Tab key={i} label={itm} style={{ color: selectedItem?.hotelDetails?.find((item) => item.languageCode === itm) ? '#107d4f' : '#d35a00' }} component={Typography} icon={<LanguageCircle />} iconPosition="start" />
                                            )
                                        })
                                    }
                                </Tabs>
