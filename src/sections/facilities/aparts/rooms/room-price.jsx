...
import PriceApartModal from 'sections/priceSections/PriceApartModal';
...
export default function RoomPriceSection() {
    ...
    useEffect(() => {
        if (isEdit) {
            setLoading(true);
            GetRoomPrices(params.id).then((res) => { setData(res.data); setIsEdit(false); setLoading(false); })
        }
    }, [isEdit])
