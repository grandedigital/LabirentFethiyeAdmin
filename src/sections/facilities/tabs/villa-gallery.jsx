        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <MainCard>
                  ...
                  <>
                    {
                      photoList.some(photo => photo.videoLink !== null) ?
                        <div style={{ width: '160px', height: '170px', float: 'left', margin: '10px', position: 'relative' }}>
                          <img width={160} height={140} style={{ border: '3px solid #999696' }} src={`${import.meta.env.VITE_APP_BACKEND_URL}/Uploads/VillaPhotos/k_${photoList.find((pht) => pht.videoLink !== null)?.image}`} alt="" />
                    ...
                    <ReactSortable tag={CustomComponent} list={photo} setList={setPhoto}>
                      {photo.map((item, index) => (
                        <div style={{ width: '160px', height: '170px', float: 'left', margin: '10px' }} key={index} data-index={index}>
                          <img src={`${import.meta.env.VITE_APP_BACKEND_URL}/Uploads/VillaPhotos/k_${item.image}`} width={160} height={140} style={{ border: '3px solid #999696' }} />
                          <Stack direction="row" spacing={0}>
                            <Tooltip title="Delete">
                              <IconButton
                                color="error"
              ...
              <PhotoModalDelete selectedItem={selectedPhotoDeleteItem} setIsEdit={setIsEdit} id={photoDeleteId} title={photoDeleteId} open={photoModalDelete} handleClose={handleClose} />
              ...
              <VideoModal villa={true} open={videoModal} modalToggler={setVideoModal} villaId={params.id} setIsEdit={setIsEdit} setLoading={setLoading} />
