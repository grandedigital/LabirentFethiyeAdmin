...
export default function FormStaticContentUpdate({ closeModal, setIsAdded, selectedItem }) {
    const user = useUser()
                ...
                if (id) {
                    ...
                    await UpdateWebPageDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Sabit İçerik düzenlendi.',
                                variant: 'alert',
                    ...
                    await CreateWebPageDetail(fd).then((res) => {
                        if (res?.statusCode === 200) {
                            openSnackbar({
                                open: true,
                                message: 'Sabit İçerik detayları eklendi.',
                                variant: 'alert',
            ...
            <FormikProvider value={formik}>
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                    <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
                        <DialogTitle>Sabit İçerik Düzenle</DialogTitle>
                        <Divider />
